...
def display_info(t):
    ...
    OLED.show()
    # 把测量好的数据存储到json文件中，提供给程序上报给后端
    # 写入 JSON 文件（覆盖写入）
    data = {
    'bmp': bmp_str.split(":")[1].strip(),
    'sop2': spo2_str.split(":")[1].strip(),
    'temp': temp_str.split(":")[1].strip()
    }
...
def get_heart_rate():
    ...
    # 创建I2C对象(检测MAX30102)
    i2c = SoftI2C(sda=Pin(21), scl=Pin(22), freq=400000)  # Fast: 400kHz, slow: 100kHz
